...
- **Tracker-Free Updates**: Discard automatic update services that include telemetry. Updates should check versions via the GitHub Releases API or a static server by comparing hashes, strictly without transmitting any hardware identifiers.
...
- **100% Offline Operation**: The software must not connect to any external APIs and offers no cloud synchronization. All reading, writing, compression, and exporting must happen within local RAM and storage.
